...
public class RIOBuildingDetector extends BuildingDetector
{
    ...
    class Simulator{
        ...
        private void initializeAir() {
            worldBounds = worldInfo.getBounds();
            
            int xSamples= (int) Math.ceil(worldBounds.getHeight() / SAMPLE_SIZE) + 1;
            int ySamples= (int) Math.ceil(worldBounds.getWidth() / SAMPLE_SIZE) + 1;
            
            maxX = xSamples * SAMPLE_SIZE;
            maxY = ySamples * SAMPLE_SIZE;
            minX = (int)worldBounds.getMinX();
            minY = (int)worldBounds.getMinY();
            
            //System.out.println("(Ix,Iy) = (" + xSamples + "," + ySamples + ")");
        ...
        */
        ...
        private Rectangle rect2 =new Rectangle(0,0,0,0);
    
        public float getLength(Point2D a, Point2D b){
            a.distance(b);
            return 0;
        }
         ...
         */
        public Point getRndPoint(Point a, Point b){
            float[] mb=simulator.getAffineFunction((float)a.x,(float)a.y,(float)b.x,(float)b.y);
        ...
        public Point getRndPoint(Point a, double length){
            Rnd rnd = new Rnd();
    ...
    public class RIOBuilding{
        ...
        public void initWallValues(){
            ...
            Enumeration e=connectedBuildings.keys();
        ...
        public double _getTemperture(){
            double rv = energy/capacity;
            
            System.out.println("eng:" + energy + " cap:" + capacity + " RV:" + rv  +
                        " cap:" + capacity + " fuel:" + fuel + " isDef:" + selfBuilding.isTemperatureDefined());
        ...
        public double getEnergy(){
            if(Double.isNaN(energy) ||energy==Double.POSITIVE_INFINITY||energy==Double.NEGATIVE_INFINITY){
    ...
    public class Wall {
        ...
        public void findHits() {
            ...
            for(int emitted=0;emitted<rays;emitted++){
                //creating ray
                Point start=simulator.getRndPoint(a,b);
                if(start==null){
                    strange++;
                    continue;
                }
                Point end=simulator.getRndPoint(start,MAX_SAMPLE_DISTANCE);
                //intersect
                Wall closest=null;
                double minDist=Double.MAX_VALUE;
                for (Wall other: simulator.allWalls){
                    if (other == this) continue;
                    Point cross = simulator.intersect(start, end, other.a, other.b);
                    if (cross != null){
