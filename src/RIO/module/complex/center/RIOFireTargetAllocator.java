package RIO.module.complex.center;
...
//import RIO.module.algorithm.RioFireClustering;
...
public class RIOFireTargetAllocator extends FireTargetAllocator {
	...
	private DynamicClustering fireClustering;
	...
	public RIOFireTargetAllocator(AgentInfo ai, WorldInfo wi, ScenarioInfo si, ModuleManager moduleManager,
		...
		case PRECOMPUTED:
			...
			this.fireClustering = moduleManager.getModule(
					"RIOFireTargetAllocator.SampleFireClustering", 
					"RIO.module.algorithm.RioFireClustering");
			break;
		case NON_PRECOMPUTE:
			this.picker = moduleManager.getCommandPicker(
					"TacticsFireStation.CommandPicker",
		...
		this.fireClustering=moduleManager.getModule("RIOFireTargetAllocator.SampleFireClustering", "RIO.module.algorithm.RioFireClustering");
	...
	private List<StandardEntity> calcInFireCluster(int currentTime,List<StandardEntity> agents){
		...
		ArrayList<EntityID> heatingBuildings = new ArrayList<>();
		ArrayList<EntityID> burningBuildings = new ArrayList<>();
		ArrayList<EntityID> infernoBuildings = new ArrayList<>();
		...
		if(!rateList.isEmpty()){
			...
			for(int i=0;i<clusterList.size();i++){
				...
				if(sortFireClusterList(fireClustering.getClusterEntityIDs(index), heatingBuildings, burningBuildings, infernoBuildings)!=null) {
					for(EntityID target:sortFireClusterList(fireClustering.getClusterEntityIDs(index), heatingBuildings, burningBuildings, infernoBuildings)){
	...
	private Collection<EntityID> sortFireClusterList(Collection<EntityID> fireClusterList,
			ArrayList<EntityID> heatingBuildings,
			ArrayList<EntityID> burningBuildings,
			ArrayList<EntityID> infernoBuildings) {
		for(EntityID entityID:fireClusterList) {
			...
			if(se instanceof Building) {
				...
				if(building.getFierynessEnum() == Fieryness.INFERNO){
					...
					heatingBuildings.add(entityID);
				}
			}
		}
		fireClusterList.clear();
		fireClusterList.addAll(heatingBuildings);
		fireClusterList.addAll(burningBuildings);
		fireClusterList.addAll(infernoBuildings);
		return fireClusterList;
	}
	//innter
	class mostValuable{
